...
import { SEARCH_EVENT_SUGGESTIONS } from '../../graphql/events';
...
interface EventSearchSuggestionsProps {
  placeholder?: string;
  onSelect: (item: EventSuggestion) => void;
...
export function EventSearchSuggestions({
  ...
  const [fetchSuggestions, { data }] = useLazyQuery(SEARCH_EVENT_SUGGESTIONS, {
    fetchPolicy: 'network-only',
    onCompleted: () => setHasSearched(true),
  });

  const suggestions: EventSuggestion[] = data?.eventSearchSuggestions || [];
  ...
  const handleSelect = (item: EventSuggestion) => {
    onSelect(item);
    setSearchTerm('');
    setShowSuggestions(false);
    setSelectedIndex(-1);
